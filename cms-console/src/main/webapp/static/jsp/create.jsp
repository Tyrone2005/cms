			...
			<tbody>
				...
				<tr class="warning">
					...
					<td><input type="text" class="form-control" id="macAddress"
						name="macAddress" placeholder="请输入MAC地址,格式如：80-00-0B-56-3B-32"></td>
				</tr>
	...
	<script>
		...
		function checkMac() {
			...
			if (!reg_name.test(macAddress)) {
				layer.msg("mac地址格式不正确,mac地址格式为00-24-21-19-BD-E4", {
					icon : 2
				});
				return false;
			}
		...
		function create() {
			...
			if (macAddress == '' || notBefore == '' || notAfter == ''
					|| licPath == '' || subject == '' || issuedTime == '') {
				layer.msg("不能为空", {
					icon : 2
				});
				return
			...
			var reg_name = /[A-F\d]{2}-[A-F\d]{2}-[A-F\d]{2}-[A-F\d]{2}-[A-F\d]{2}-[A-F\d]{2}/;
			if (!reg_name.test(macAddress)) {
				layer.msg("mac地址不正确,mac地址格式为00-24-21-19-BD-E4", {
					icon : 2
				});
				return
